...
if __name__ == '__main__':
    ...
    X_train = X[:int(x/100*len(X))]
    y_train = y[:int(x/100*len(X))]

    X_test = X[int(x/100*len(X)):]
    y_test = y[int(x/100*len(X)):]
    ...
    all_preds = [clf.predict(X) for clf in clfs]

    final_preds = []
    for i in range(len(X_test)):
        votes = {}
        for j in range(len(clfs)):
            predicted_class = all_preds[j][i]
            if predicted_class not in votes:
                votes[predicted_class] = 0
            votes[predicted_class] += weights[j]
